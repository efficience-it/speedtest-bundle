...
var settings = {
	...
	test_order: "ID_U___P", //order in which tests will be performed as a string. D=Download, U=Upload, P=Ping+Jitter, I=IP, _=1 second delay
	...
	count_ping: 300, // number of pings to perform in ping test
	url_dl: location.origin + "/speedtest/generate-chunks", // path to a large file or garbage.php, used for download test. must be relative to this js file
	...
	url_getIp: location.origin + "/speedtest/get-ip", // path to getIP.php relative to this js file, or a similar thing that outputs the client's ip
...
function getIp(done) {
	...
	xhr.open("GET", settings.url_getIp + '/' + Math.random(), true);
	xhr.send();
}
	...
	var testStream = function(i, delay) {
		setTimeout(
			function() {
				...
				xhr[i].open("GET", settings.url_dl + '/' + Math.random(), true); // random string to prevent caching
				xhr[i].send();
			}.bind(this),
			1 + delay
		);
